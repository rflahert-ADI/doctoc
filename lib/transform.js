...
function matchesStart(line) {
  return (/<!-- START doctoc /).test(line) || (/{\/\* START doctoc /).test(line);
}

function matchesEnd(line) {
  return (/<!-- END doctoc /).test(line) || (/{\/\* END doctoc /).test(line);
}
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, title, notitle, entryPrefix, processAll, updateOnly, mdx = false) {
  start = `${mdx ? mdxExt[0] : mdExt[0]} START doctoc generated TOC please keep comment here to allow auto update ${mdx ? mdxExt[1] : mdExt[1]}\n` +
            `${mdx ? mdxExt[0] : mdExt[0]} DON\'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE ${mdx ? mdxExt[1] : mdExt[1]}`
  , end   = `${mdx ? mdxExt[0] : mdExt[0]} END doctoc generated TOC please keep comment here to allow auto update ${mdx ? mdxExt[1] : mdExt[1]}`
  , skipTag = `<!-- DOCTOC SKIP -->`
  , skipTagMDX = `{/* DOCTOC SKIP */}`;
  if (content.indexOf(skipTag) !== -1 || content.indexOf(skipTagMDX) !== -1)  return { transformed: false };
